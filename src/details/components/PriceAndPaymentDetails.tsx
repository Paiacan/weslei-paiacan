      ...
      <div className="relative flex flex-col w-screen h-auto">
        <div className=" mt-10 w-full h-full lg:mt-0 flex flex-col lg:flex-row justify-start items-center">
          ...
          <div className="flex flex-col h-auto w-[80%] lg:h-[60%] lg:w-1/4 items-center justify-center m-10">
            ...
            <h2 className="text-center text-2xl text-white m-4">
              SEU TREINO <span className="span-text">VITALÍCIO</span> ESTÁ A UM{" "}
              <a
                className="hover:text-[var(--on-primary-color)] animate-pulse"
                href={isConsultancy ? EXTERNAL_LINKS.WHATSAPP : "#"}
                onClick={() => trackOnBuyButtonClick()}
                target="_blank"
              >
                CLICK
              </a>
          ...
          <div className="flex flex-col">
            <img
              className="w-[97%] lg:w-auto"
              src={bonusImage}
              alt="bonus-img"
            />
            <h2 className="self-center mt-20 mb-10 text-red-600 text-6xl decoration-white">
              <span className="relative">
                {`R$ ${oldPrice.toFixed(2).toString().replace(".", ",")}`}
                <span className="absolute inset-0 top-1/2 left-0 w-full border-t-4 border-white rotate-[-10deg]"></span>
              </span>
            </h2>
            <h2 className="self-center mb-20 text-white text-6xl text-shadow">{`R$ ${actualPrice
              .toFixed(2)
              .toString()
              .replace(".", ",")}`}</h2>
          </div>
        ...
        <img
          className="w-[80%] lg:w-[50%] self-center"
          src={paymentMethods}
          alt="payment-methods"
        />
      </div>
